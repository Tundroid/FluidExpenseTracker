package com.moleculesoft.fluidfinanceassistant.data.viewmodel;
...
public class SharedViewModel<T> extends ViewModel {
                ...
                response -> {
                    switch (menu) {
                        case CATEGORY:
                            ...
                            break;
                        case INCOME:
                            processIncomeResponse(response, callback);
                            break;
                        default:
    ...
    public void processIncomeResponse(JSONArray response, FetchCallback callback) {
        try {
            List<Income> fetchedItems = new ArrayList<>();
            JSONObject item;
            for (int i = 0; i < response.length(); i++) {
                item = response.getJSONObject(i);

                fetchedItems.add(new Income(
                        item.getInt("IncomeID"),
                        item.getString("IncomeDate"),
                        item.getInt("Amount"),
                        item.getString("CategoryName")
                        , item.getString("IncomeDescription")
                ));
            }

            fetchedItems.sort((income1, income2) -> {
                // Compare by date in descending order
                Date date1 = Util.getDateObject(income1.getIncomeDate());
                Date date2 = Util.getDateObject(income2.getIncomeDate());
                int dateComparison = date1 != null && date2 != null ? date2.compareTo(date1) : 0;

                // If dates are the same, compare by description in ascending order
                if (dateComparison == 0) {
                    return income1.getIncomeDescription().compareToIgnoreCase(income1.getIncomeDescription());
                }
                return dateComparison;
            });
    ...
    public void processCategoryResponse(JSONArray response, FetchCallback callback) {
        try {
            ...
            fetchedItems.sort((category1, category2) -> {
